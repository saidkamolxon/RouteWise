...
public class GoogleMapsApiBroker(IConfiguredClients clients) : IGoogleMapsApiBroker
{
    private readonly IRestClient client = clients.GoogleMapsClient;
    ...
    public async Task<string> GetDistanceAsync(string origin, string destination, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var result = await GetDistanceMatrixResultAsync(parameters, cancellationToken);
    ...
    private async Task<DistanceMatrixResult> GetDistanceMatrixResultAsync(Dictionary<string, string> parameters, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<object> GetGeocodingAsync(string location, bool reverse, CancellationToken cancellationToken = default)
    {
        ...
        return await GetResultAsync("geocode/json", parameters, cancellationToken);
    }
    ...
    public async Task<string> GetReverseGeocodingAsync(string coordinates, CancellationToken cancellationToken = default)
    {
        ...
        var response = await GetResultAsync("geocode/json", parameters, cancellationToken);
    ...
    public async Task<string> GetStaticMapAsync(string center, string[] objects, CancellationToken cancellationToken = default)
    {
        ...
        var url = client.BuildUri(CreateNewRestRequest("staticmap", parameters)).ToString();
